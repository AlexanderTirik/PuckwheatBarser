...
def parse_fozzy(response):
    ...
    for product in products:
        try:
            ...
            product_url = description.find("div", class_="h3 product-title").a.get(
                "href"
            )
            img_url = product.find("div", class_="thumbnail-container").a.img.get("src")
            weight = (
                product.find("div", class_="product-reference text-muted")
                .a.get_text()
                .replace("Фасовка: ", "")
                .strip()
            )
...
def parse_epicentrk(response):
    ...
    for product in products:
        try:
            ...
            price = fixed_price_format(price)
            source = (
                product.find("ul", class_="card__characteristics")
                .find_all("li")[1]
                .get_text()
                .replace("Бренд:", "")
                .strip()
            )
            product_url = EPICENTRK_HOST + product.find("a", class_="card__photo").get(
                "href"
            )
...
def parse_auchan(response):
    ...
    for product in products:
        try:
            ...
            product_url = AUCHAN_HOST + product.find("a", class_="product-tile").get(
                "href"
            )
            img_url = product.find("img", class_="product-tile__image-i").get("src")
            weight = (
                product.find("div", class_="product-tile__title-wrapper")
                .find("div")
                .get_text()
            )
            weight = clean_weight(weight)
